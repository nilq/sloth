...
impl TypeTab {
    ...
    pub fn set_type(&self, index: usize, env_index: usize, t: Type) -> CheckResult<()> {
        if env_index == 0 {
            ...
            match types.get_mut(index) {
                ...
                None => Err(CheckError::new(&format!("can't set type of invalid type index: {}", index))),
            }
        } else {
            match self.parent {
                Some(ref p) => p.set_type(index, env_index - 1, t),
                None => Err(CheckError::new(&format!("can't set type with invalid env index: {}", env_index))),
            }
        }
    }

    pub fn get_type(&self, index: usize, env_index: usize) -> CheckResult<Type> {
        if env_index == 0 {
            match self.types.borrow().get(index) {
                Some(v) => Ok(v.clone()),
                None    => Err(CheckError::new(&format!("can't get type of invalid type index: {}", index))),
            }
        } else {
            match self.parent {
                Some(ref p) => p.get_type(index, env_index - 1),
                None => Err(CheckError::new(&format!("can't get type with invalid env index: {}", index))),
            }
        }
    }
